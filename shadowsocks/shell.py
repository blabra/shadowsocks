...
def get_config(is_local):
    ...
    config['pid-file'] = config.get('pid-file', '/data/data/com.termux/files/usr/var/run/shadowsocks.pid')
    config['log-file'] = config.get('log-file', '/data/data/com.termux/files/usr/var/log/shadowsocks.log')
    config['verbose'] = config.get('verbose', False)
    config['local_address'] = to_str(config.get('local_address', '127.0.0.1'))
    config['local_port'] = config.get('local_port', 1090)
